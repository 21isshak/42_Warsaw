...
{
	input = input.substr(input.find_first_not_of(' '), input.find_last_not_of(' ') - input.find_first_not_of(' ') + 1);  // trim of spaces
...
bool ScalarConverter::isValidNumber(std::string& input)
{
	unsigned int i = 0;
	int dotCount = 0;
	while ((input.at(i) == '+' || input.at(i) == '-') && i < input.size())
		i++;
	while (i < input.size())
	{
...
bool ScalarConverter::isPseudo(std::string& input)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		if (input.compare(pseudoliterals[i]) == 0)
			return true;
	}
	return false;
}
...
bool ScalarConverter::isPrintableASCII(int i)
{
	return (i >= 32 && i <= 126);
}
...
{
	if ((value > std::numeric_limits<double>::max() || value < -std::numeric_limits<double>::max()) && isPseudo(input) == false)
	{
...
void ScalarConverter::printFloat(double& value, std::string& input)
{
	...
	if ((value > static_cast<double>(std::numeric_limits<float>::max()) || value < -static_cast<double>(std::numeric_limits<float>::max())) && isPseudo(input) == false)
	{
...
{
	if (value > std::numeric_limits<int>::max() || value < std::numeric_limits<int>::min() || isPseudo(input))
	{
...
void ScalarConverter::printChar(double& value, std::string& input)
{
	...
	if (value > std::numeric_limits<char>::max() || value < std::numeric_limits<char>::min() || isPseudo(input))
	{
...
double ScalarConverter::stringToDouble(std::string& str)
{
	...
	if (isPseudo(str))
	{
		for (; index < 4; index++)
		{
			if (str.compare(pseudoliterals[index]) == 0)
				break;
		}
		switch (index)
		{
